...
detector = cv2.CascadeClassifier('Features\haarcascade_frontalface_default.xml')
#Haar Cascade classifier is an effective object detection approach.0
...
while True:
    ...
    for (x,y,w,h) in faces:
        ...
        cv2.imwrite("Features\samples/face." + str(face_id) + '.' + str(count) + ".jpg", converted_image[y:y+h,x:x+w])
    ...
    if k == 27: # Press 'ESC' to stop
        break
    elif count >= 50 : #  (More sample --> More accuracy)
         break
